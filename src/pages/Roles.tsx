...
import { Card, Text, Title, Badge, Button, TextInput, NumberInput, Textarea } from '@tremor/react';
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="flex justify-between items-end mb-6 mt-2">
                <div>
                    <Text className="font-semibold text-tremor-content-strong">Roles Disponibles</Text>
                </div>
                <Button onClick={() => { setEditingRole(null); setForm({ nombre: '', descripcion: '', permisosIds: [] }); setShowModal(true); }} icon={Plus}>
                    Nuevo Rol Global
                </Button>
            </div>
            ...
            {roles.length === 0 ? (
                <EmptyState
                    icon={<Shield className="w-8 h-8 text-tremor-content" />}
                    ...
                    action={
                        <Button onClick={() => { setEditingRole(null); setForm({ nombre: '', descripcion: '', permisosIds: [] }); setShowModal(true); }} icon={Plus} className="mt-4">
                            Crear mi primer rol
                        </Button>
                    }
                />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {roles.map(role => (
                        <Card key={role.id} className="p-0 flex flex-col group relative overflow-hidden transition-all hover:shadow-md">
                            {role.es_sistema && (
                                <div className="absolute top-0 right-0 bg-tremor-background-subtle border-b border-l border-tremor-border text-tremor-content-subtle text-[10px] font-bold px-3 py-1 rounded-bl-lg tracking-wider uppercase">
                                    Sistema / Inmutable
                                </div>
                            )}
                            <div className="flex-1 p-5 pt-7">
                                <div className="flex items-center gap-3 mb-3">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border ${role.es_sistema ? 'bg-tremor-background-subtle border-tremor-border' : 'bg-emerald-50 border-emerald-100'}`}>
                                        <Shield className={`w-5 h-5 ${role.es_sistema ? 'text-tremor-content-subtle' : 'text-emerald-500'}`} />
                                    </div>
                                    <div>
                            ...
                            <div className="p-4 pt-3 border-t border-tremor-border bg-tremor-background-subtle flex items-center justify-between">
            ...
            <Modal open={showModal} title={editingRole ? (editingRole.es_sistema ? 'Viendo permisos del sistema' : 'Editar Rol') : 'Nuevo Rol Global'} onClose={() => setShowModal(false)} size="lg">
                <div className="space-y-5 pt-2">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <TextInput value={form.nombre} disabled={editingRole?.es_sistema} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} placeholder="Ej: Operador Jr" />
                        </div>
                        <div>
                            <Text className="mb-1 font-medium">Nivel de Seguridad (0 = SuperAdmin)</Text>
                            <NumberInput min={0} value={editingRole?.nivel || 10} disabled={true} placeholder="Nivel de acceso (10 = base)" />
                        </div>
                        <div className="md:col-span-2">
                            <Text className="mb-1 font-medium">Descripción</Text>
                            <Textarea disabled={editingRole?.es_sistema} value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))} placeholder="Qué puede hacer este rol..." rows={3} />
                        </div>
                    </div>

                    <div>
                        <div className="flex items-center justify-between mb-3 border-b border-tremor-border pb-2">
                            ...
                            {permisos.map(p => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex items-start gap-3 p-3.5 rounded-xl border text-left transition-all ${isChecked ? 'border-emerald-500 bg-emerald-50/30' : 'border-tremor-border hover:border-tremor-content-subtle hover:bg-tremor-background-subtle'}`}
                                    >
                                        <div className={`mt-0.5 w-[18px] h-[18px] rounded flex items-center justify-center flex-shrink-0 transition-colors ${isChecked ? 'bg-emerald-500 text-white' : 'bg-white border-2 border-tremor-border'}`}>
                                            {isChecked && <Check className="w-3.5 h-3.5" strokeWidth={3} />}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className={cn("text-xs font-bold capitalize truncate", isChecked ? "text-emerald-900" : "text-tremor-content-strong")}>{p.recurso}: {p.accion}</p>
                                            <p className="text-[10px] text-tremor-content leading-tight mt-1 line-clamp-2">{p.descripcion}</p>
                                        </div>
                                    </button>
                                );
                            })}
                ...
                {!editingRole?.es_sistema && (
                    <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-tremor-border">

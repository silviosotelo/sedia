...
export const roleService = {
    async createRole(tenantId: string | null, nombre: string, descripcion: string, nivel: number, permisosIds: string[]): Promise<string> {
    ...
    async updateRole(roleId: string, tenantId: string | null, data: { nombre?: string; descripcion?: string; permisosIds?: string[] }): Promise<void> {
        return withTransaction(async (client) => {
            // Verificar acceso
            const role = await client.query('SELECT tenant_id FROM roles WHERE id = $1', [roleId]);
            if (role.rows.length === 0) throw new Error('Rol no encontrado');
    ...
    async deleteRole(roleId: string, tenantId: string | null): Promise<void> {
        const role = await queryOne<{ tenant_id: string | null, es_sistema: boolean }>('SELECT tenant_id, es_sistema FROM roles WHERE id = $1', [roleId]);
        if (!role || role.tenant_id !== tenantId || role.es_sistema) {

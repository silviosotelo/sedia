...
// Mock mode disabled — all mock references below are dead code
...
export const api = {
  ...
  clasificacion: {
    ...
    aplicar: (tenantId: string): Promise<{ message: string; etiquetas_aplicadas: number }> => {
      ...
      return request<{ data: { message: string; etiquetas_aplicadas: number } }>(`/tenants/${tenantId}/clasificacion/aplicar`, { method: 'POST', body: JSON.stringify({}) }).then((r) => r.data);
    },
  },
  ...
  platformSifen: {
    getConfig: () =>
      request<{ data: any }>('/system/sifen-config').then((r) => r.data),
    updateConfig: (body: Record<string, unknown>) =>
      request<{ data: any }>('/system/sifen-config', { method: 'PUT', body: JSON.stringify(body) }).then((r) => r.data),
  },

  sifenRef: {
    get: (table: string) =>
      request<{ data: any[] }>(`/sifen-ref/${table}`).then((r) => r.data),
    departamentos: () =>
      request<{ data: any[] }>('/sifen-ref/departamentos').then((r) => r.data),
    distritos: (departamento?: number) =>
      request<{ data: any[] }>(`/sifen-ref/distritos${departamento ? `?departamento=${departamento}` : ''}`).then((r) => r.data),
    ciudades: (params: { distrito?: number; search?: string }) => {
      const qs = new URLSearchParams();
      if (params.distrito) qs.set('distrito', String(params.distrito));
      if (params.search) qs.set('search', params.search);
      return request<{ data: any[] }>(`/sifen-ref/ciudades?${qs}`).then((r) => r.data);
    },
  },
